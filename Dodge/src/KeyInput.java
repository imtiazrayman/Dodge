...
public class KeyInput extends KeyAdapter{
        ...
        public void keyPressed(KeyEvent e){
                ...
                for(int i = 0; i < handler.object.size(); i++){
                        GameObject tempObject = handler.object.get(i); // on additional
        ...
        public void keyReleased(KeyEvent e){
                ...
                for(int i = 0; i < handler.object.size(); i++){
                        ...
                        if(tempObject.getId() == ID.Player2){
                        	if(key == KeyEvent.VK_W) { tempObject.setVelY(-5); keyDown[0] = false;  }
                            if(key == KeyEvent.VK_S) { tempObject.setVelY(5); keyDown[1] = false; }
                            if(key == KeyEvent.VK_D) { tempObject.setVelX(5); keyDown[2] = false; }
                            if(key == KeyEvent.VK_A) { tempObject.setVelX(-5); keyDown[3] = false; }
                            if(!keyDown[0] && !keyDown[1]) tempObject.setVelY(0);
                            if(!keyDown[2] && !keyDown[3]) tempObject.setVelX(0);
                    }
                }
